...
class Config:
    ...
    specific_file_path: str = None  # <-- Add this line
    exposure_time: float = 0.15  # s
    min_points_for_fit: int = 10
    min_length: int = 2 # Parameter for minimum length
    min_histogram_bins: int = 10
    max_td_value: float = 10e4  # s
...
def filter_ambiguous_lengths(df: pd.DataFrame, min_length: int) -> pd.DataFrame:
    """
    Filter out groups with ambiguous lengths.
    
    Args:
        df: DataFrame containing localization data with 'group' and 'frame' columns.
        min_length: Minimum length required for a group to be considered valid.
    
    Returns:
        pd.DataFrame: Filtered DataFrame with only valid groups.
    """
    # Calculate lengths for each group
    lengths = df.groupby('group')['frame'].agg(lambda x: x.max() - x.min() + 1)
    
    # Identify valid groups
    valid_groups = lengths[lengths >= min_length].index
...
def fit_cumulative_frequency(df: pd.DataFrame, config: Config) -> pd.DataFrame:
    ...
    for group in unique_groups:
        ...
        # Check for minimum points for fit
        if len(group_data) < config.min_points_for_fit:
            df_with_td.loc[df_with_td['group'] == group, 'Td'] = np.nan
            df_with_td.loc[df_with_td['group'] == group, 'Tb'] = np.nan
            failed_fits += 1
            continue

        #  Calculate the length of the group
        first_frame = group_data['frame'].min()
        last_frame = group_data['frame'].max()
        ...
        length_values = group_data['len'].values * config.exposure_time * 1000

        # Use fixed binning strategy with parameters from Config
        hist_dark, bin_edges_dark = np.histogram(
            dark_values, 
        ...
        bin_centers_dark = (bin_edges_dark[:-1] + bin_edges_dark[1:]) / 2

        # Calculate cumulative frequency 
        cum_freq_dark = np.cumsum(hist_dark) * np.diff(bin_edges_dark)
        
        try:
            # Initial parameter guesses
            p0_td = [0.01, 0.3, 100]
            popt_td, _ = curve_fit(exp_func, bin_centers_dark, cum_freq_dark, p0=p0_td)
            
            # Add Td to dataframe for this group
            df_with_td.loc[df_with_td['group'] == group, 'Td'] = popt_td[2]
            successful_fits += 1
        ...
        hist_len, bin_edges_len = np.histogram(
            length_values, 
            bins=np.arange(config.bin_min, config.bin_max, config.bin_step), 
            density=True
        )
        ...
        cum_freq_len = np.cumsum(hist_len) * np.diff(bin_edges_len)

        try:
            # Initial parameter guesses
            p0_tb = [0.01, 0.3, 100]
            popt_tb, _ = curve_fit(exp_func, bin_centers_len, cum_freq_len, p0=p0_tb)
            df_with_td.loc[df_with_td['group'] == group, 'Tb'] = popt_tb[2]
            
        except (RuntimeError, ValueError):
            #print(f"Warning: Could not fit exponential function for group {group}: {str(e)}")
            df_with_td.loc[df_with_td['group'] == group, 'Tb'] = np.nan

    # Filter out ambituous lengths
    df_with_td = filter_ambiguous_lengths(df_with_td, config.min_length)
...
def save_all_results(df: pd.DataFrame, yaml_docs: List, config: Config, original_name: str, num_sample_fits: int = 10) -> Tuple[Path, Path, Path]:
    ...
    footer_template = {
        'TdCDF calculated using TdCDF_of_single_clusters_v7.py': {
            'Td [s]': float(df['Td'].mean()),
            'Tb [ms]': float(df['Tb'].mean()),
    ...
    # single_row_df = df.groupby('group').agg({
    #     'Td': 'first',
    #     'Tb': 'first'
    #     **{col: 'first' for col in df.columns if col not in ['Td', 'Tb']}
    # }).reset_index()
...
def process_all_files(config: Config):
    ...
    try:
        ...
        hdf5_files = [f for f in all_hdf5_files if f.name.endswith('_filtered_in.hdf5') and 'centers' not in f.name]    
...
def main():
    ...
    config = Config(
        input_dir=INPUT_DIR,
        specific_file_path=SPECIFIC_FILE_PATH
    )
